...
public interface ProjectManagerRole {
    ...
    @EntityAttributePolicy(entityClass = User.class, attributes = {"id", "avartar", "version", "username", "password", "email", "active", "timeZoneId"}, action = EntityAttributePolicyAction.VIEW)
    @EntityPolicy(entityClass = User.class, actions = {EntityPolicyAction.READ, EntityPolicyAction.UPDATE})
    void user();

    @MenuPolicy(menuIds = {"User.list", "TimeEntry.list", "Project.list", "Task_.list", "ProjectStats.list"})
    @ViewPolicy(viewIds = {"User.list", "TimeEntry.list", "Project.list", "Task_.list", "ProjectStats.list", "Project.detail", "Task_.detail", "TimeEntry.detail", "User.detail"})
    void screens();
}